...
const propertyParentIsSkipped = (path, options) => {
  ...
  if (pathProps.length > 1) {
    ...
    for (const pathProp of pathProps) {
      ...
      const parentObject = getPropertyByPath(options, parentPath) || getPropertyByName(options, parentPath) || {};
...
const shouldSkipProperty = (path, options) => (propertyIsSkipped(path, options) || propertyParentIsSkipped(path, options))
