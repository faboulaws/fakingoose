...
function shouldSkip(path, options) {
  ...
  if (pathProps.length > 1) {
    ...
    for (let pathPropIndex = 0; pathPropIndex < pathProps.length; pathPropIndex++) {
      if (pathPropIndex === 0) {
        parentPath = pathProps[pathPropIndex];
      } else {
        parentPath = `${parentPath}.${pathProps[pathPropIndex]}`;
      }
      if (get(options, `${parentPath}.skip`, false) || get(options, `['${parentPath}'].skip`, false)) {
        return true;
      }
    }
  }
