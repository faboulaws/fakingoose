...
const generators = {
  ...
  decimal128: (pathDef, { tostring = true } = {}) => (tostring ? `${chance.floating()}` : chance.floating()),
  ...
  map: (pathDef, options) => {
    ...
    const mock = keys.reduce(
      (obj, key) => Object.assign(obj, { [key]: generator(pathDef, options) }),
      {},
    );
    return mock;
  },
    ...
    if (!path.includes('$*')) {
      if (typeof get(options, `${path}.value`) === 'function' || typeof get(options, `['${path}'].value`, undefined) === 'function') {
        delayedFieldGeneration.push(
          () => populateField(
            mockObject, schema, path, options, get(staticFields, path, undefined),
          ),
        );
      } else {
